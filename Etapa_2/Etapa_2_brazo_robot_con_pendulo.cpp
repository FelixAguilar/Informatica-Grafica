// Etapa_2_mano_robot_con_pendulo.cpp
...
const GLfloat angulo_inicial = 25.0f;
const GLfloat gravedad = 9.81f;
const GLfloat longitud = 4.0f;
...
GLfloat fAngulo5 = angulo_inicial; // Angulo del pendulo.
GLfloat ciclo = 0;
GLfloat caida = 0;
bool cae = false;
...
const GLfloat radius_pendulum = 0.05f;
...
void drawScene() {
	...
	glTranslatef(
		...
		(sin(toRadians(fAngulo1)) * 0.52) + (sin(toRadians(fAngulo2)) * 0.52) + (sin(toRadians(fAngulo3)) * 0.17f) - caida,
		0.0f);

	glTranslatef(-0.8f + 0.52, -0.2f, 0.0f);
	glRotatef(fAngulo5, 0.0f, 0.0f, 1.0f);
	...
	{
		glVertex2d(0.0f + radius_pendulum * cos(angleTool), -0.40f + radius_pendulum * sin(angleTool));
...
void Timer(GLint t) {
	...
	if (cae) {
		...
		// Angulo pendulo.
		fAngulo5 = angulo_inicial * cos(toRadians(sqrt(gravedad / longitud) * ciclo));
		ciclo++;
